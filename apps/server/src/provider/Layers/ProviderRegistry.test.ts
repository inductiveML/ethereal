...
const TestClaudePtyRuntimeLive = Layer.mergeAll(
  Layer.succeed(PtyAdapter.PtyAdapter, {
    spawn: () =>
      Effect.die(new Error("Claude PTY spawn is not expected in provider registry tests")),
  }),
...
it.layer(Layer.mergeAll(NodeServices.layer, ServerSettingsModule.layerTest(), TestHttpClientLive))(
  ...
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
      ...
      it("preserves previously discovered provider models when a refresh returns none", () => {
        const previousProvider = {
          ...
          driver: ProviderDriverKind.make("fixture"),
          status: "ready",
          enabled: true,
          installed: true,
          auth: { status: "authenticated" },
      ...
      it("persists merged provider snapshots for the providers that were refreshed", () => {
        ...
        const mergedProviders = mergeProviderSnapshots(previousProviders, [refreshedFixture]);
        const persistedProviders = selectProvidersByKind(
          mergedProviders,
          new Set([ProviderDriverKind.make("fixture")]),
        );
      ...
      it.effect("persists the merged snapshot when a live update has empty models", () =>
        Effect.gen(function* () {
          const fixtureDriver = ProviderDriverKind.make("fixture");
          const fixtureInstanceId = ProviderInstanceId.make("fixture");
          const initialProvider = {
          ...
          const instance = {
            instanceId: fixtureInstanceId,
            driverKind: fixtureDriver,
            continuationIdentity: {
              driverKind: fixtureDriver,
              continuationKey: "fixture:instance:fixture",
            },
            displayName: undefined,
            enabled: true,
            snapshot: {
              maintenanceCapabilities: makeManualOnlyProviderMaintenanceCapabilities({
                provider: fixtureDriver,
                packageName: null,
              }),
              getSnapshot: Effect.succeed(initialProvider),
          ...
          const instanceRegistryLayer = Layer.succeed(
            ...
            {
              getInstance: (instanceId) =>
                Effect.succeed(instanceId === fixtureInstanceId ? instance : undefined),
              listInstances: Effect.succeed([instance]),
              listUnavailable: Effect.succeed([]),
              streamChanges: Stream.empty,
      ...
      // This test intentionally avoids a command-aware mock so the real
      ...
      it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
        Effect.gen(function* () {
          ...
          const providerRegistryLayer = ProviderRegistryLive.pipe(
            ...
            Layer.provideMerge(TestClaudePtyRuntimeLive),
            Layer.provideMerge(
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in Ethereal settings.");
        }),
      );
    });
    ...
    describe("checkClaudeProviderStatus", () => {
      it.effect("reports the installed CLI and version-derived model catalog", () =>
        Effect.gen(function* () {
      ...
      it.effect("hides models unsupported by the installed Claude Code version", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(defaultClaudeSettings);
          assert.strictEqual(
            status.models.some((model) => model.slug === "claude-fable-5"),
      ...
      it.effect("runs the version probe with the configured Claude HOME", () => {
        const claudeHome = "/tmp/ethereal-claude-home";
      ...
      it.effect("returns an error without leaking stderr when the version probe fails", () => {
        ...
        return Effect.gen(function* () {
          ...
          Effect.provide(
            mockSpawnerLayer((args) => {
              if (args.join(" ") === "--version") {
                return { stdout: "", stderr: secretStderr, code: 1 };
              }
              throw new Error(`Unexpected args: ${args.join(" ")}`);
            }),
          ),
      ...
      it.effect("skips Claude probes when the provider is disabled", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(disabledClaudeSettings);
          assert.strictEqual(status.enabled, false);
          assert.strictEqual(status.status, "disabled");
          assert.strictEqual(status.installed, false);
        }).pipe(Effect.provide(failingSpawnerLayer("Claude should not spawn"))),
      );
    });
  },
);
