...
This guide is for people who want to use more than one Codex account in Ethereal.
...
- both accounts can see the same Ethereal/Codex sessions
...
In Ethereal Settings, name it something obvious:
...
In Ethereal Settings, add another Codex provider:
...
Ethereal shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Ethereal treats it
